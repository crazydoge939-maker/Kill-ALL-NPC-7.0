...
local function killNpc(npc)
	...
	local humanoid = npc:FindFirstChildOfClass("Humanoid")
	if humanoid then
		humanoid.Health = 0 -- Убиваем через здоровье
		humanoid.MaxHealth = 0 -- Удаляем максимум, чтобы не было восстановления
		-- Можно дополнительно вызвать событие смерти, если нужно
		humanoid:TakeDamage(humanoid.Health)
	end
	-- Также, чтобы убедиться, что NPC удален или "убит", можно отключить SimulationRadius
	local simRadius = npc:FindFirstChild("SimulationRadius")
	if simRadius then
		-- Например, отключить его или изменить радиус
		simRadius.MaximumSimulationRadius = 0
	end
end
